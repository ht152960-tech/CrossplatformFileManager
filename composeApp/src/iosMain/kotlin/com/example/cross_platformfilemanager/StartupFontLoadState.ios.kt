...
// iOS 端当前不单独探测异步字体失败，只记录诊断能力缺失。
@Composable
actual fun rememberAppFontLoadState(): AppFontLoadState {
    LaunchedEffect(Unit) {
        reportStartupTrace("uiFont status=unsupported failureDetectionSupported=false platform=ios")
    }
...
actual fun rememberFullCjkFontLoadState(): FullCjkFontLoadState {
    LaunchedEffect(Unit) {
        reportStartupTrace("fullCjkFont status=unsupported failureDetectionSupported=false platform=ios")
    }
